...
import { Package, RefreshCw, ArrowLeftRight, X, History, AlertCircle, Wallet, ChevronRight } from 'lucide-react';
            ...
            <section>
              ...
              <ul className="space-y-2">
                {visibleHistory.map(h => (
                  <HistoryCard key={h.id} holding={h} />
                ))}
              </ul>
            </section>
          )}
...
function EmptyState() {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white px-6 py-12 text-center">
      ...
      <h3 className="text-base font-semibold text-slate-800">You currently hold no containers yet</h3>
      <p className="text-sm text-slate-500 mt-1.5 max-w-sm mx-auto">
        Steel containers shipped with your orders will appear here. Return them for a deposit
        refund, or refill them in your next order to skip the deposit.
      </p>
    </div>
  );
}
...
function HistoryCard({ holding }: { holding: HistoryHolding }) {
  return (
    <li className="rounded-lg border border-slate-200 bg-white px-4 py-3 flex items-center justify-between gap-3">
      <div className="min-w-0">
