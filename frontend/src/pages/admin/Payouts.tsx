...
import {
  ...
  Loader2, Wallet, Users as UsersIcon,
} from 'lucide-react';
      ...
      <div className="card">
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-slate-900">Traders & Bank Accounts</h3>
                      ...
                      <td>
                        {linked ? (
                          <div className="flex flex-col gap-0.5">
                            <span className="badge bg-emerald-100 text-emerald-700 w-fit">
                              <Check size={10} /> Linked
                            </span>
                            <span className="text-xs font-mono text-slate-400">{t.razorpay_linked_account_id}</span>
                          </div>
                        ) : hasBank ? (
                          <span className="badge bg-amber-100 text-amber-700">Not linked</span>
                        ) : (
                          <span className="badge bg-slate-100 text-slate-500">No bank</span>
                        )}
                      </td>
                      ...
                      <td>
                        {linked ? (
                          <button
                            ...
                            className="btn-ghost text-xs text-brand-600 hover:text-brand-700 font-semibold"
                          >
                            View commissions
                          </button>
                        ) : (
                          <button
                            ...
                            disabled={!hasBank || busyTraderId === t.id}
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-brand-600 hover:bg-brand-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {busyTraderId === t.id
                              ? <Loader2 size={12} className="animate-spin" />
                              : <Link2 size={12} />}
                            {busyTraderId === t.id ? 'Linking...' : 'Create Linked Account'}
                          </button>
                        )}
                      </td>
                    </tr>
